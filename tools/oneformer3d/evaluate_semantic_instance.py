...
def compute_averages(aps, pr_rc, options, class_labels):
    ...
    avg_dict['all_rec'] = np.nanmean(pr_rc[1, :, o_all_but25])
    avg_dict['all_rec_25%'] = np.nanmean(pr_rc[1, :, o25])
    avg_dict['classes'] = {}
    for (li, label_name) in enumerate(class_labels):
        ...
        avg_dict['classes'][label_name]['rec50%'] = np.average(pr_rc[1, li,
                                                                  o50])
        avg_dict['classes'][label_name]['rec'] = np.average(pr_rc[1, li,
                                                                  o_all_but25])
        avg_dict['classes'][label_name]['rec25%'] = np.average(pr_rc[1, li,
                                                                   o25])
    return avg_dict
