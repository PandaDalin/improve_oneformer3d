...
def instance_seg_eval(gt_semantic_masks,
    ...
    header = ['classes', 'AP', 'AP_0.50', 'AP_0.25', 'AR', 'AR_0.50', 'AR_0.25']
    rows = []
    for label, data in metrics['classes'].items():
        aps = [data['ap'], data['ap50%'], data['ap25%'], data['rec'], data['rec50%'], data['rec25%']]
        rows.append([label] + [f'{ap:.4f}' for ap in aps])
    aps = metrics['all_ap'], metrics['all_ap_50%'], metrics['all_ap_25%'], metrics['all_rec'], metrics['all_rec_50%'], metrics['all_rec_25%']
